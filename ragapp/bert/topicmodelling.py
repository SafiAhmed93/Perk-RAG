...
def run_topic_modeling(documents: List[Dict[str, Any]], num_topics: int = None):
    ...
    print(f"Avg document length (chars): {sum(len(doc) for doc in docs) / max(1, len(docs)):.1f}")
...
def save_results(topic_model: BERTopic, topics: List[int], documents: List[Dict[str, Any]], output_dir: str):
    """
    ...
    results_df = pd.DataFrame({
        ...
        'topic': topics,
        'topic_name': [topic_model.get_topic_info().iloc[i]['Name'] if i < len(topic_model.get_topic_info()) else 'Unknown' 
    ...
    # Save visualizations
    topic_model.visualize_topics().write_html(os.path.join(output_dir, 'topic_visualization.html'))
    topic_model.visualize_barchart(top_n_topics=10).write_html(os.path.join(output_dir, 'top_topics_barchart.html'))
    
    # Generate and save document-topic map
    fig, ax = plt.subplots(figsize=(12, 10))
    topic_model.visualize_documents(docs=[doc['content'] for doc in documents], topics=topics, ax=ax)
